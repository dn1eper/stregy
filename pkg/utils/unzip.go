...
func unzip_all(source, destination string) error {
	return filepath.Walk(source, func(_ string, info os.FileInfo, err error) error {
		if err != nil {
			log.Fatalf(err.Error())
		}
		filepath := path.Join(source, info.Name())
		unzipSource(filepath, destination)
		return nil
	})
}
